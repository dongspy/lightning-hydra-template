...
class MlpModule(LightningModule):
    ...
    def __init__(
        ...
        self.precision = torchmetrics.Precision(task="binary", num_classes=num_classes, average='macro')
        # self.recall = torchmetrics.Recall(task="binary", num_classes=num_classes, average='macro')
        # self.f1 = torchmetrics.F1Score(task="binary", num_classes=num_classes, average='macro')
        # self.auroc = torchmetrics.AUROC(task="binary", num_classes=num_classes, average='macro')
    ...
    def training_step(
        ...
        self.log('train_precision', self.precision, on_step=False, on_epoch=True, prog_bar=True)
        # self.log('train_recall', self.recall(preds, targets))
        # self.log('train_f1', self.f1(preds, targets))
        # 对于AUROC，需要softmax或sigmoid的输出概率
        
        # self.log('train_auroc', self.auroc(probs, targets))
